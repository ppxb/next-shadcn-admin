...
const itemVariants = cva(
	'relative flex flex-row items-center gap-2 rounded-lg p-2 text-start text-muted-foreground wrap-anywhere transition-colors hover:bg-accent/50 hover:text-accent-foreground/80 hover:transition-none [&_svg]:size-4 [&_svg]:shrink-0',
	{
		variants: {
			variant: {
				link: 'data-[active=true]:bg-primary/10 data-[active=true]:text-primary data-[active=true]:hover:transition-colors'
			},
			highlight: {
...
export function SidebarDrawer({
	...
	return (
		<>
			<Base.SidebarDrawerOverlay className="fixed z-40 inset-0 backdrop-blur-xs data-[state=open]:animate-na-fade-in data-[state=closed]:animate-na-fade-out" />
			<Base.SidebarDrawerContent
				className={cn(
					'fixed text-[0.9375rem] flex flex-col shadow-lg border-s end-0 inset-y-0 w-[85%] max-w-[380px] z-40 bg-background data-[state=open]:animate-na-sidebar-in data-[state=closed]:animate-na-sidebar-out',
					className
				)}
				{...props}
			>
